...
class HouseholdMembers:
    ...
    def __init__(self, name, labour_data):
        ...
        self.wage_day = self.wage_year / 365    # Assume work 365 days per year
        self.wage_hour = self.wage_day / 14     # Assume work 12 hrs day

        self.value_of_labour_year = self.awake_year * self.wage_hour    # Calculation for total labour value
    ...
    def agricultural_labour_calc(self):
        ...
        # How to differentiate sowing time from growing season from harvest time? Attach to form object for calcs
        # ====================================================
    ...
    def domestic_labour_calc(self):
        ...
        # Currently only for normal years. Need to define drought years and attach to form object then add if statement
...
def test_economics_algorithms(form):
    ...
    # if livestock model has been run
    # then import livestock data and get total yearly manure production data for forward run
    # ==========================
    if form.livestock_run:
        ...
        for management_type, calcs in manure_data.items():
            ...
            for calc_method, livestock in calcs.items():
                ...
                # ==========================================
                manure_fr = [prod_data['manure_prod_fr'] for animal, prod_data in livestock.items()]
                total_manure_fr = [sum(ic) for ic in zip(*manure_fr)]     # Sum all manure production to get total produced each year for all animals
                calc_method_manure_dic.update({calc_method: total_manure_fr})   # Update dictionary with calculation method total production
    ...
    # ====================
    pcc_alpha_0 = 9.194     # Intercept
    pcc_alpha_1 = 0.0154    # Full Household income
    pcc_alpha_2 = 0.0351    # Land
    pcc_alpha_3 = -0.0000816    # Land squared
    pcc_alpha_4 = 0.0211    # Total land utilised
    pcc_alpha_5 = -0.000396 # Total land utilised squared
    pcc_alpha_6 = -0.399    # Log - Household size adult equivalents
    pcc_alpha_7 = 1         # Regional price index
    ...
    for calc_method, calcs in all_subareas_full_hh_dic.items():
        fr_pcc = []
        for year in calcs:  # year is FHI for each year
    ...
    # ====================
    rfi_alpha_0 = -1.782    # Intercept
    rfi_alpha_1 = -0.0333   # Full Household income
    rfi_alpha_2 = -0.102    # Land
    rfi_alpha_3 = 0.000222  # Land squared
    rfi_alpha_4 = -0.008400 # Total land utilised
    rfi_alpha_5 = 0         # Total land utilised squared - NO DATA SO REMOVED FROM EQUATION
    rfi_alpha_6 = 0.802     # Log - Household size adult equivalents    #
    rfi_alpha_7 = 1         # Regional price index - CHECK!!!!
    ...
    for calc_method, calcs in all_subareas_full_hh_dic.items():
        ...
        for year in calcs:   # year is FHI for each year
            ...
            year_rfi = norm.cdf(year_rfi)   # Calculate probability to return value between 0 and 1
    ...
    # ==================================
    dd_alpha_0 = 1.782      # Intercept
    dd_alpha_1 = 0.000182   # Full Household income
    dd_alpha_2 = 0.00718    # Land
    dd_alpha_3 = -0.0000185 # Land squared
    dd_alpha_4 = 0.00544    # Total land utilised
    dd_alpha_5 = 0.0269     # Agriculutrual Diversity (Number of crops)
    dd_alpha_6 = 0.0784     # Log - Household size adult equivalents
    dd_alpha_7 = 1          # Regional price index - CHECK!!!!
...
#            data.update({'full_hh_income_miami': calcs})
    ...
    # Start with full household income
    # ================================
